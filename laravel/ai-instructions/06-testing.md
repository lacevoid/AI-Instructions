...
- Domain logic tests.
- Mirror source structure: `tests/Unit/Actions/Group/CreateGroupActionTest.php`.

### Feature Tests (`tests/Feature/`)
- HTTP endpoint tests.
- Full integration tests with database.
- Mirror route/feature structure: `tests/Feature/Content/ArticleTest.php`.
...
class ArticleTest extends TestCase
{
    ...
    public function user_can_view_articles_index(): void
    {
        $this->actingAs($this->user)
            ->get(route('dashboard.content.articles.index'))
            ->assertOk();
    }
    ...
    {
        $this->actingAs($this->user)
            ->post(route('dashboard.content.articles.store'), [...])
            ->assertRedirect(route('dashboard.content.articles.index'));
